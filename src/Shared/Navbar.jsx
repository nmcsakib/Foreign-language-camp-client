    ...
    <div>
      <div className="navbar w-11/12 md:w-3/4 z-20 lg-py-0 bg-purple-500/30 backdrop-blur-md rounded-lg fixed left-1/2 bottom-10 transform -translate-x-1/2 ">
        ...
        <div className="navbar-center flex grow-1">
          <ul className="menu menu-horizontal space-x-5">
            ...
            <li className='tooltip tooltip-bottom' data-tip="Home">
              <NavLink to="/" className={({ isActive }) => isActive ? "transform translate-y-[-10px] rounded-lg p-3 text-md bg-purple-500 shadow inner-md " : "text-slate-900 rounded-lg p-3 text-md bg-purple-500 shadow inner-md"}>
                <FaHome className='text-2xl' />
              </NavLink>
            </li>
            {/* Classes */}
            <li className='tooltip tooltip-bottom' data-tip="Classes">
              <NavLink to="/active-classes" className={({ isActive }) => isActive ? "transform translate-y-[-10px] rounded-lg p-3 text-md bg-purple-500 shadow inner-md " : "text-slate-900 rounded-lg p-3 text-md bg-purple-500 shadow inner-md"}>
                <FaChalkboardTeacher className='text-2xl' />
              </NavLink>
            </li>
            {/* Instructors */}
            <li className='tooltip tooltip-bottom' data-tip="Instructors">
              <NavLink to="/all-instructors" className={({ isActive }) => isActive ? "transform translate-y-[-10px] rounded-lg p-3 text-md bg-purple-500 shadow inner-md " : "text-slate-900 rounded-lg p-3 text-md bg-purple-500 shadow inner-md"}>
        ...
        <div className="navbar-end">
          {
            user ?
              <div className='transition-all flex space-x-4'>
                {/* Dashboard */}
                ...
                <div className='tooltip tooltip-bottom' data-tip={`${user.displayName}`}>
                  <img className='btn btn-circle text-2xl transform hover:translate-y-[-10px] text-md shadow inner-md' src={user?.photoURL} />
                </div>
                {/* Log out */}
                <button onClick={handleLogOut} data-tip="Log out" className="tooltip tooltip-bottom btn btn-circle text-slate-900 transform hover:bg-purple-500/30 p-3 text-2xl bg-purple-500 shadow inner-md" >
                  <FaSignOutAlt />
                </button>
              </div>
              :
